...
public class WeeklySalary extends AppCompatActivity {
    ...
    private boolean validateDailySalary() {
        String dailySalary = dailySalaryOfStaff.getEditText().getText().toString().trim();
        if (dailySalary.isEmpty()) {
    ...
    public void dailySalary(View v) {
        ...
        while (res1.moveToNext()) {
            if (res1.getString(1).equals(GlobalVariable.emp_mob_no)) {
                ...
                Integer daily = Integer.parseInt(dailySalaryOfStaff.getEditText().getText().toString().trim());

                String name = res1.getString(0);
                String mobno = res1.getString(1);

                String salaryType = res1.getString(2);
                Integer salary = daily;
